...
export class PanierComponent implements OnInit {
    ...
    updateQuantity(item: any, delta: number): void {
        ...
        if (newQty <= 0) {
            ...
            this.cartService.updateQuantity(item.produitId._id || item.produitId, newQty, item.variationId).subscribe();
        }
    }

    removeFromCart(item: any): void {
        this.cartService.removeFromCart(item.produitId._id || item.produitId, item.variationId).subscribe(() => {
            this.snackBar.open('Produit retiré du panier', 'Annuler', {
                duration: 4000
            }).onAction().subscribe(() => {
                this.cartService.addToCart(item.produitId._id || item.produitId, item.variationId, item.quantite).subscribe();
            });
        });
    }
