...
export class CheckoutComponent implements OnInit {
    ...
    private checkDateAvailability(dateStr: string, type: string): boolean {
        ...
        const isJourJ = date.getFullYear() === today.getFullYear() &&
            date.getMonth() === today.getMonth() &&
            date.getDate() === today.getDate();

        if (isJourJ && type !== 'livraison_boutique') {
            this.snackBar.open(`La livraison pour aujourd'hui n'est possible qu'en livraison directe par boutique.${nextDateMsg}`, 'Fermer', {
                duration: 6000,
            });
            return false;
        }
        ...
        if (isJourJ && type === 'livraison_boutique') {
            ...
            if (!boutique || !boutique.accepteLivraisonJourJ) {
                this.snackBar.open(`Cette boutique n'accepte pas les livraisons pour aujourd'hui.${nextDateMsg}`, 'Fermer', {
                    duration: 6000,
                });
                return false;
            }
