...
export class AppProduitEditComponent implements OnInit, OnDestroy {
  isLoading = false;
  isSubmitting = false;
  serverError = '';
  selectedImages: File[] = [];
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(
      this.route.paramMap.subscribe((params) => {
        ...
        if (!id) {
          this.serverError = 'Produit introuvable.';
          return;
        }
        this.productId = id;
        this.loadProduct(id);
  ...
  submit(): void {
    ...
    formData.append('boutiqueId', value.boutiqueId?.trim() ?? '');
    formData.append('titre', value.titre?.trim() ?? '');
    formData.append('categorieId', value.categorieId?.trim() ?? '');
    ...
    if (this.selectedImages.length) {
      ...
      formData.append('replaceImages', String(value.replaceImages));
    }
    ...
    this.productService
      ...
      .subscribe({
        next: (response) => {
          const message = response?.message ?? 'Produit mis a jour';
          this.snackBar.open(message, 'Fermer', { duration: 3000 });
          const updatedId = response?.data?._id ?? this.productId;
          this.router.navigate(['/produits', updatedId]);
        },
        error: (error) => {
  ...
  resetImages(): void {
    this.selectedImages = [];
    this.form.patchValue({ replaceImages: false });
    this.cdr.markForCheck();
  }
  ...
  private loadProduct(id: string): void {
    ...
    this.productService
      ...
      .subscribe({
        ...
        error: (error) => {
          this.serverError = error?.error?.message ?? 'Impossible de charger le produit.';
        },
      });
  }

  private populateForm(product: ProductCreateResponse): void {
    ...
    this.form.patchValue({
      ...
      sousCategoriesIds: (product.sousCategoriesIds ?? []).join(', '),
      tags: (product.tags ?? []).join(', '),
      attributs: product.attributs ? JSON.stringify(product.attributs, null, 2) : '',
      prixBaseActuel: product.prixBaseActuel ?? null,
      stockQuantite: product.stock?.quantite ?? null,
      stockSeuilAlerte: product.stock?.seuilAlerte ?? null,
      hasVariations: Boolean(product.hasVariations),
      estActif: product.estActif !== undefined ? Boolean(product.estActif) : true,
      sku: product.sku ?? '',
      replaceImages: false,
    });
  ...
  private updateCategoryLabel(): void {
    ...
    this.selectedCategoryLabel = this.categoryPathMap.get(categorieId) ?? '';
  }
