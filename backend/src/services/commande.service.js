...
export const createCommande = async (userId, deliveryData) => {
    ...
    const isJourJ = requestedDate.getFullYear() === today.getFullYear() &&
        requestedDate.getMonth() === today.getMonth() &&
        requestedDate.getDate() === today.getDate();

    if (isJourJ && typedelivery !== 'livraison_boutique') {
        throw createError('La livraison au Jour J n\'est possible que pour la livraison directe par boutique.', 400);
    }

    if (typedelivery === 'livraison_boutique') {
        if (boutiqueIds.length > 1) {
            throw createError('La livraison par boutique n\'est possible que si tous les produits proviennent de la même boutique.', 400);
        }
    ...
    const boutiquesData = await Promise.all(boutiqueIds.map(async bId => {
        ...
        const itemsInBoutique = await Promise.all(panier.items
    ...
    for (const bId of boutiqueIds) {
        await notifyBoutiqueUsers(bId, `🔔 Nouvelle Commande #${newCommande.numeroCommande}`, `Une nouvelle commande ${newCommande.numeroCommande} nécessite votre attention.`, { commandeId: newCommande._id });
    }
    await notifyAdmins(`🆕 Commande #${newCommande.numeroCommande} passée`, `Une nouvelle commande ${newCommande.numeroCommande} a été passée par un client.`, { commandeId: newCommande._id });
...
export const getCommandeById = async (commandeId, userId, role) => {
    const commande = await Commande.findById(commandeId).populate('clientId', 'nom prenom email telephone');
    ...
    if (role !== 'admin') {
        ...
        if (role === 'boutique') {
            ...
            const isBoutiqueInOrder = commande.boutiques.some(b => b.boutiqueId.toString() === user.boutiqueId?.toString());
...
export const markBoutiqueDeliveredToDepot = async (commandeId, boutiqueId) => {
    ...
    await notifyAdmins(`🚚 Dépôt éffectué - #${commande.numeroCommande}`, `La boutique ${commande.boutiques[bIndex].name} a déposé ses articles pour la commande ${commande.numeroCommande}.`, { commandeId: commande._id });
...
export const confirmDepotReceipt = async (commandeId, boutiqueId, adminId) => {
    ...
    const allArrived = commande.boutiques.every(b => (b.depotEntrepot.dateValidation || b.status === 'annulee'));
    if (allArrived) {
...
export const cancelOrder = async (commandeId, userId, role, reason) => {
    ...
    if (role === 'admin') {
        ...
        const boutique = commande.boutiques.find(b => b.boutiqueId.toString() === boutiqueId?.toString());
        ...
        if (anyValidated) throw createError('Certains articles sont déjà en cours de livraison et ne peuvent plus être annulés.', 400);
...
export const cancelOrderItem = async (commandeId, boutiqueId, produitId, userId, role, reason) => {
    if (role === 'admin') {
        throw createError('Un administrateur ne peut qu\'annuler toute la commande, pas des articles spécifiques.', 403);
    }
    ...
    if (boutique.depotEntrepot?.dateValidation) {
        throw createError('La commande est déjà confirmée à l\'entrepôt et ne peut plus être modifiée.', 400);
    }

    // Identify Item (consider potential variationId too if possible, but for now produitId)
    const item = boutique.items.find(i => i.produitId.toString() === produitId.toString() && i.status !== 'annulee');
    ...
    const anyActiveItems = commande.boutiques.some(b => b.items.some(it => it.status !== 'annulee'));
    if (!anyActiveItems) {
        commande.statusLivraison = 'annulee';
    }
...
export const confirmFinalReceipt = async (commandeId, userId, role) => {
    ...
    await notifyAdmins('Commande terminée', `La commande ${commande.numeroCommande} a été confirmée comme reçue par le client.`, { commandeId: commande._id });
